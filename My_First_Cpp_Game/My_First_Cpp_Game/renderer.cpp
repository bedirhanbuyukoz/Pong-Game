...
internal void//Ekran Deðiþtirme menuden oyuna geçiþ
...
draw_rect_in_pixels(int x0, int y0, int x1, int y1, u32 color) {
	...
	for (int y = y0; y < y1; y++) {//Renkleri renderlama
...
internal void //Geniþ ekrana uyarlama çökme sorunu çözümü
...
internal void//Raketlerin Hareket Sorunu Çözümü
draw_rect(float x, float y, float half_size_x, float half_size_y, u32 color) {
	...
	// Pixellerin Yer Deðiþtirme (Daha Güzel görüntü)
	int x0 = x - half_size_x;
	int x1 = x + half_size_x;
	int y0 = y - half_size_y;
	int y1 = y + half_size_y;

	draw_rect_in_pixels(x0, y0, x1, y1, color);//Raket Konumlandýrma
}

//Giriþteki Harfleri Çektiðimiz Karakter (Menü Ýçin)
...
internal void //TEXTLERÝ YAZDIRMA
...
internal void
draw_number(int number, float x, float y, float size, u32 color) {  //Skorlarý Yazdýrma
	float half_size = size * .5f;

	bool drew_number = false; //0-0 Ekranda Gösterme (Oyun baþlangýcýnda)
	while (number || !drew_number) { 
		drew_number = true; 

		int digit = number % 10; // Çok Basamaklý Yazdýrma
		number = number / 10;   //Çok Basamaklý Yazdýrma
